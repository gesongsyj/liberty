...
CREATE TABLE `currency` (
  ...
  `buyPoint` tinyint(1) DEFAULT NULL,
...
CREATE TABLE `kline` (
  ...
  `max` double(20,10) NOT NULL COMMENT '最大值',
  `min` double(20,10) NOT NULL COMMENT '最小值',
  `open` double(20,10) NOT NULL,
  `close` double(20,10) NOT NULL,
  `currencyId` int(11) NOT NULL COMMENT '币种id',
  `strokeId` int(11) DEFAULT NULL COMMENT '笔id',
  `type` char(1) DEFAULT NULL COMMENT 'K线级别',
  `bar` double(20,10) DEFAULT NULL COMMENT '红绿柱的高度',
  `ema_s` double(20,10) DEFAULT NULL,
  `ema_l` double(20,10) DEFAULT NULL,
  `diff` double(20,10) DEFAULT NULL,
  `dea` double(20,10) DEFAULT NULL,
  PRIMARY KEY (`id`),
...
) ENGINE=InnoDB AUTO_INCREMENT=2939306 DEFAULT CHARSET=utf8;
...
CREATE TABLE `line` (
  ...
  `direction` char(1) DEFAULT NULL COMMENT '线段方向,上/下',
  PRIMARY KEY (`id`),
